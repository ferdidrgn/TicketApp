...
class _BaseCardContainer extends StatelessWidget {
  ...
  const _BaseCardContainer({required this.child,
    this.onTap,
    this.borderRadius,
    this.width,
    this.padding,
...
class _SearchPageState extends ConsumerState<SearchPage> {
  ...
  Widget build(final BuildContext context) {
    ...
    // "Tümü" modunda her kategoriden ilk 10 öğe, diğer modlarda sayfalama
    final data = (
  ...
  Widget _buildHeader(final String query) =>
      Padding(
  ...
  List<Widget> _buildFilteredSection(
      ...
      switch (_selectedFilter) {
        ...
        2 => [
          // Eski "if loading return ShimmerGrid else return RealGrid" yerine
          // Artık sadece Grid var, içi duruma göre değişiyor.
          ...
          SliverPadding(
            ...
            sliver: SliverGrid(
              ...
              delegate: SliverChildBuilderDelegate(
                ...
                // Yükleniyorsa 9 tane sahte kart, yoksa gerçek sayı
...
class _GridCards {
  // Ortak Kart Yapısı (Stack + Gradient + Text) - ESKİ HALİ
  static Widget _cardStack(final String img, final String title,
      final String badge,
  ...
  static Widget verticalLarge(final BuildContext context, final dynamic item,
      final bool isStage) =>
      _BaseCardContainer(
        onTap: () =>
            Navigator.push(
          ...
          builder: (final context, final constraints) =>
              Stack(
                children: [
                  ...
                  // Dinamik gradient overlay (farklı renk tonları)
                  Positioned.fill(
                    child: Container(
                      decoration: BoxDecoration(
...
class _HorizontalListSection extends StatelessWidget {
  ...
  const _HorizontalListSection({required this.title,
...
class _PlayerSection extends StatelessWidget {
  ...
  @override
  Widget build(final BuildContext context) {
    // Eğer yükleniyorsa 6 tane, yüklendiyse oyuncu sayısı kadar öğe olsun
...
class ArtisticBrushChip extends StatefulWidget {
  ...
  const ArtisticBrushChip({super.key,
    required this.text,
...
class _CornerLinePainter extends CustomPainter {
  ...
  void paint(final Canvas canvas, final Size size) {
    ...
    final path = Path()
      ...
      ..lineTo(0, 0)..lineTo(size.width * 0.6, 0);
