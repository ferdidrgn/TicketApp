...
class ProfilePage extends ConsumerWidget {
  ...
  Widget build(final BuildContext context, final WidgetRef ref) {
    final User? firebaseUser = LoginService().currentUser;

    return Scaffold(
      body: ListView(
        padding: const EdgeInsets.all(20.0),
        children: [
          _buildProfileCard(firebaseUser, context),
          ...
          if (firebaseUser != null) ...[
            CustomElevatedButton(
              ...
              onPressed: () => _navigateTo(
                  UserProfileEditScreen(userId: firebaseUser.uid), context),
            ),
            CustomElevatedButton(
              text: 'Biletlerim',
              iconData: Icons.theaters_rounded,
              onPressed: () =>
                  _navigateTo(MyTicketPage(userId: firebaseUser.uid), context),
            ),
            CustomElevatedButton(
          ...
          CustomElevatedButton(
            ...
            // Callback fonksiyonlarını burada tanımlıyoruz
  ...
  Future<void> _signOut(final context) async {
    try {
      await LoginService().signOut(); // LoginService'dan çıkış yap
      _navigateToLogin(context);
    } catch (e) {
